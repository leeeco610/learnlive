...
+function ($) {
  ...
  var Calendar = function (params) {
      ...
      p.updateValue = function () {
          p.wrapper.find('.picker-calendar-day-selected').removeClass('picker-calendar-day-selected').find("i").remove();
          var i, inputValue;
          for (i = 0; i < p.value.length; i++) {
              var valueDate = new Date(p.value[i]);
              var $selectedDate = p.wrapper.find('.picker-calendar-day[data-date="' + valueDate.getFullYear() + '-' + valueDate.getMonth() + '-' + valueDate.getDate() + '"]');
              $selectedDate.addClass('picker-calendar-day-selected');
              $selectedDate.find("i").length == 0 && $selectedDate.append('<i>休</i>');
          }
      ...
      p.initCalendarEvents = function () {
          ...
          function handleDayClick(e) {
              ...
              if(!day.hasClass('picker-calendar-day-selected')){
                  ...
                  day.find("i").length == 0 && day.append('<i>休</i>');
              }
      ...
      p.monthHTML = function (date, offset) {
          ...
          for (i = 1; i <= rows; i++) {
              ...
              for (j = 1; j <= cols; j++) {
                  ...
                  rowHTML += '<div data-year="' + dayYear + '" data-month="' + dayMonth + '" data-day="' + dayNumber + '" class="picker-calendar-day' + (addClass) + '" data-date="' + (dayYear + '-' + dayMonth + '-' + dayNumber) + '"><span>'+dayNumber+'</span>'+xiuHtml+'</div>';
              }
...
+ function($) {
    ...
    $.attachInfiniteScroll = function(infiniteContent) {
        $.getScroller(infiniteContent) && $.getScroller(infiniteContent).on('scroll', handleInfiniteScroll);
    };
    $.detachInfiniteScroll = function(infiniteContent) {
        $.getScroller(infiniteContent) && $.getScroller(infiniteContent).off('scroll', handleInfiniteScroll);
    };
